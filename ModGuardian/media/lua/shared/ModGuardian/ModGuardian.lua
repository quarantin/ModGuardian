...
local function reportModpack(config)
	...
	local params = {
		...
		workshopID = modInfo and modInfo:getWorkshopID()
		originalWorkshopID = config.workshopID
	}
...
local function ModGuardianCheck()
	...
	if not ModGuardian or not ModGuardian.config then
		error("ModGuardian configuration is not set!")
	end
	...
	elseif not config.disableExclusiveServers and config.exclusiveServersEnabled and not config.exclusiveServers[getServerIP()] then
...
local function ModGuardianConfig(config)
	...
	config.exclusiveServersEnabled = emptyTable(config.exclusiveServers)
